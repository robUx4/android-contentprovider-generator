 ...
 * <p>Used to read/write {@link TeamModel} into a database
 * and select items using a {@link org.jraf.androidcontentprovidergenerator.sample.provider.team.TeamSelection}</p>
 */
public class TeamDatabaseModelHandler extends DatabaseModelHandler<TeamModel, TeamCursor, TeamSelection> {
    /**
     * Default Constructor.
     */
    public TeamDatabaseModelHandler() {
        this(TeamDatabaseSerializer.INSTANCE);
    }
     ...
     */
    public TeamDatabaseModelHandler(DatabaseSerializer<TeamModel, TeamCursor> serializer) {
        super(serializer);
    }

    @Override
    protected TeamSelection getItemSelection(@NonNull TeamModel itemToSelect) {
        return new TeamSelection(itemToSelect);
    }
}