 ...
 * <p>Used to read/write {@link CompanyModel} into a database
 * and select items using a {@link org.jraf.androidcontentprovidergenerator.sample.provider.company.CompanySelection}</p>
 */
public class CompanyDatabaseModelHandler extends DatabaseModelHandler<CompanyModel, CompanyCursor, CompanySelection> {
    /**
     * Default Constructor.
     */
    public CompanyDatabaseModelHandler() {
        this(CompanyDatabaseSerializer.INSTANCE);
    }
     ...
     */
    public CompanyDatabaseModelHandler(DatabaseSerializer<CompanyModel, CompanyCursor> serializer) {
        super(serializer);
    }

    @Override
    protected CompanySelection getItemSelection(@NonNull CompanyModel itemToSelect) {
        return new CompanySelection(itemToSelect);
    }
}